...
LEEDOX가 상위 브랜드가 되면서 실제 운영 Platform과 학습자가 직접 수정하는 실습 코드를 분리하는 방향을 채택했습니다.

앞으로 이 커리큘럼은 Todo, 게시판, 문서와 알림 같은 공통 기능을 서로 다른 예제로 만들지 않고, 작은 팀을 위한 하나의 협업 SaaS에 누적하는 흐름으로 개편합니다. 학습 코드는 별도의 Lab 저장소에서 챕터별 시작 branch와 완료 tag로 제공하는 것을 목표로 합니다.

현재 이 장의 아키텍처와 비즈니스 모델에는 기존 Platform 구현이 함께 설명되어 있습니다. 새 Lab의 최종 정책으로 확정된 내용은 아니므로, 전환이 완료될 때까지 운영 서비스의 현재 상태와 향후 학습 모델을 구분해 읽어야 합니다.

상세한 방향은 [Chatdox 학습 상품과 실습 저장소 전략](../chatdox/chatdox_learning_product_strategy.md)을 참고하세요.
...
|------|---------|-----------|------------------|
| **상태** | Guest | 로그인 후 7일 이내 | 월 ₩9,900 결제 중 |
...
| 사용자 | 기본 화면 | 주요 메뉴 | 목적 |
|---|---|---|---|
| 게스트 (비로그인) | 랜딩페이지 | 회원가입, 문서 | 서비스 소개와 공개 문서 체험 |
| 회원 / Trial | 대시보드 | 문서, 마이페이지, 로그아웃 | 무료 체험과 학습 진행 |
| 구독자 | 대시보드 | 문서, 이용현황, 결제관리, 로그아웃 | 전체 콘텐츠 이용과 구독 관리 |
...
```text
게스트   → 랜딩페이지 → 회원가입
회원     → 대시보드   → Trial 문서(1~5장) → 구독하기
구독자   → 대시보드   → 전체 문서(1~20장) → 이용현황/결제관리
어드민   → 관리자 대시보드 → 종합현황/문서/사용자관리
```

세부 규칙과 구현은 다음 문서에서 다룹니다.

- [07장 인증](07_authentication.md): 로그인·로그아웃과 인증 상태
- [08장 권한](08_authorization.md): Guest/Trial/Subscriber/Admin 접근 정책
- [09장 결제](09_payment.md): 구독 결제와 결제 상태
- [10장 사용자 대시보드](10_dashboard.md): Trial·구독자 화면과 이용현황
...
| **결제** | Toss Payments / PortOne | 선택형 | 구독 결제 처리 |
...
Phase 2: 핵심 기능 구현 (챕터 6~16)
  ...
  9.  Payment (Toss Payments / PortOne)
  10. 사용자 대시보드
  11. 관리자 대시보드
  12. Email & 알림
