...
`claudox/` (formerly `CLAUDE/`, and before that `Working/`) is the running record of Tommy's collaboration with Claude ("Claudox"), written as a 20-chapter story. Purpose: every activity between Tommy and Claudox gets recorded here, chapter by chapter, to become a learning resource for users who want to actively leverage AI — not just a personal log. Known files and their format:
...
- **memory_seed.md** — portable snapshot of my (Claudox's) cross-session memory (user identity, standing rules, project vision, root motivation). This travels with `git clone`; the real memory system under `~/.claude/projects/.../memory/` does not (it's local to this PC/user profile — see [08_qna.md](claudox/08_qna.md)).
...
## service-desk/ folder (repo root, not inside claudox/)
...
- **Scope is explicit-only**: unlike the `claudox/` TOC coverage rule (which captures *every* conversation), service-desk only tracks requests Tommy actually files (via `new.sh`/`new.ps1` or by hand). Don't auto-create tickets for things discussed in chat.
- **No Ticket by default** (rule changed 2026-07-13 via REQ 0021, superseding the REQ 0012 "ticket everything" interim rule): don't file a ticket for every doc/code change. File one only when it's genuinely useful to track — see `chatdox/git_document_guidelines.md` section 5 for the "Remote에 남길 가치가 높은 경우 / 로컬만으로 충분한 경우" checklist. If it's unclear whether something warrants a ticket, ask Tommy rather than deciding unilaterally.
...
1. Read `claudox/memory_seed.md` immediately and treat its content as current context.
...
- Whenever I apply a rule documented here (e.g. matching the claudox/ file formats above), say so briefly so Tommy can see it's kicking in — don't apply it silently.
...
- **Keep memory_seed.md in sync**: whenever a new fact/rule gets saved to local memory, mirror it into `claudox/memory_seed.md` in the same session — otherwise a clone on another machine bootstraps from a stale snapshot.
...
- **Bare chapter number shorthand**: when Tommy refers to a chapter by number alone (e.g. "01", "3", "19") with no other qualifier, he means `claudox/NN_slug.md`. When he asks to evaluate/finish/commit "01"-style, resolve it against `setup.md`/`88_progress.md` rather than asking which file he means.
...
- **Keep 97_commands.md in sync**: whenever a new shorthand keyword gets established (like `SYNC` or the bare-number convention), add a row to `claudox/97_commands.md` in the same session.
